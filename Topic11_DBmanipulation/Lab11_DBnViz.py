...
for wellID in range(1,18):
    ...
    prodDF = pd.read_sql_query(f"SELECT * FROM Rates WHERE wellID={str(wellID)};", conn)
    ...
    axes.plot(prodDF['time'], prodDF['Cum_model']/1000, color="red", ls='-', lw=3, label = 'well '+str(wellID) )
    axes.legend(loc=4)
...
>>>>>>> 916577518f417684808c4edb4aab6dff4a815814
conn.close()
